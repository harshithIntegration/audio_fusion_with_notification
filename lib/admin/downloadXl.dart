...
//     final responseEmployees = await http.post(Uri.parse(
//         'http://13.201.213.5:4040/admin/fetchallemployee'));

//     if (responseAttendance.statusCode == 200 && responseEmployees.statusCode == 200) {
//       final Map<String, dynamic> attendanceData = json.decode(responseAttendance.body);
//       final Map<String, dynamic> employeeData = json.decode(responseEmployees.body);
...
//         final List<String> attendanceEmployeeCodes = sortedData
//             .map((e) => e['emp_Code']?.toString() ?? '')
//             .toList();
...
Future<String> downloadExcel(DateTime startDate, DateTime endDate) async {
  ...
  try {
    ...
    final responseEmployees = await http.post(Uri.parse(
        'http://13.201.213.5:4080/admin/fetchallemployee'));
    ...
    if (responseAttendance.statusCode == 200 && responseEmployees.statusCode == 200) {
      ...
      if (body != null && body is List) {
        ...
        for (var date = startDate; date.isBefore(endDate) || date.isAtSameMomentAs(endDate); date = date.add(Duration(days: 1))) {
          dateRange.add("${date.day.toString().padLeft(2, '0')}-${date.month.toString().padLeft(2, '0')}-${date.year}");
        }

        for (var date in dateRange) {
          DateTime currentDate = DateTime.parse("${date.split('-')[2]}-${date.split('-')[1]}-${date.split('-')[0]}");
          String dayOfWeek = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][currentDate.weekday % 7];
          ...
          for (var employee in sortedData) {
            ...
            if (dayAndDate != null && dayAndDate is List) {
              ...
              if (!found) {
                ...
                sheet.getRangeByIndex(rowIndex, 4).setText(date);
                sheet.getRangeByIndex(rowIndex, 5).setText(dayOfWeek);
                sheet.getRangeByIndex(rowIndex, 6).setText(dayOfWeek == 'Sunday' ? 'Holiday' : 'Absent');
                rowIndex++;
              }
            }
          }
